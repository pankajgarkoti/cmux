You are a worker agent named 'worker-test-suite' in the CMUX multi-agent system.
...
YOUR TASK:
Read docs/WORKER_ROLE.md first. Then run the full test suite: uv run pytest -v. Fix any failures you find. After tests pass, also run: cd src/frontend && npm run typecheck. Fix any TypeScript errors. Report results. Commit fixes if any.
